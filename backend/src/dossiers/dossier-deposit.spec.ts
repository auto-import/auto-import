...
const deposit = {
  treasuryAccountId: '22222222-2222-4222-8222-222222222222',
  amount: 120,
  currency: 'DZD',
  officeId,
  paymentMethod: 'CASH',
...
describe('Dossier deposit currency and office', () => {
  it.each(['USD', 'CNY', 'DZD'])(
    'accepts %s in the transition DTO',
    async (currency) => {
      expect(
        await validate(
          plainToInstance(DepositTransitionDataDto, { ...deposit, currency }),
        ),
      ).toEqual([]);
    },
  );
  it('rejects an unsupported currency and invalid office ID', async () => {
    const errors = await validate(
      plainToInstance(DepositTransitionDataDto, {
        ...deposit,
        currency: 'EUR',
        officeId: 'invalid',
      }),
  ...
  function setup(validOffice = true) {
    const dossier = {
      ...
      organizationId: 'org',
      clientId: 'client',
      type: DossierType.VEHICLE_SALE_CIF as DossierType,
      workflowVersion: 2,
    ...
    const tx: any = {
      ...
      dossier: {
        ...
        update: jest
          ...
          .mockImplementation(({ data }) =>
            Object.assign(dossier, { status: data.status }),
          ),
      },
      ...
      office: {
        findFirst: jest
          .fn()
          .mockResolvedValue(validOffice ? { id: officeId } : null),
      },
      exchangeRate: {
        findFirst: jest
          .fn()
          .mockResolvedValue({ id: 'rate', rate: new Prisma.Decimal(35) }),
      },
      payment: {
        create: jest
          .fn()
      ...
      customerDeposit: {
        create: jest.fn().mockImplementation(({ data }) => {
          const record = {
            id: 'deposit',
            ...data,
            office: { id: officeId, name: 'Persisted office' },
          };
          dossier.customerDeposits.push(record);
          return record;
        }),
      },
      financeTransaction: {
      ...
      dossierStatusHistory: {
        create: jest.fn().mockResolvedValue({ id: 'history' }),
      },
    };
    tx.$transaction = jest.fn((callback) => callback(tx));
    const sync: any = {
      assertTransitionAllowed: jest.fn(),
      syncForTransition: jest.fn(),
    };
    const service = new DossiersService(
      tx,
      new DossierWorkflowService(),
      sync,
      { markEvidenceRelied: jest.fn() } as any,
      new DossierStatusPropagationService(sync, { syncFromDossier: jest.fn() } as never),
      undefined,
      undefined,
      new FinanceProjectionService(tx),
      new ExchangeRatesService(tx),
    );
    return { service, tx, dossier };
  }
  it.each(['USD', 'CNY', 'DZD'])(
    'persists the selected office and %s payment with a DZD snapshot',
    async (currency) => {
      const { service, tx } = setup();
      await service.updateStatus(
        'dossier',
        { status: 'depositReceived', deposit: { ...deposit, currency } },
        'actor',
        'org',
      );
      expect(tx.office.findFirst).toHaveBeenCalledWith({
        where: { id: officeId, organizationId: 'org', status: 'active' },
        select: { id: true },
      });
      ...
      expect(ledger.exchangeRateSnapshot.toString()).toBe(
        currency === 'DZD' ? '1' : '35',
      );
      expect(ledger.amountDzd.toString()).toBe(
        currency === 'DZD' ? '120' : '4200',
      );
      if (currency === 'DZD')
      ...
      expect(reopened.sections.finance.deposits[0].office?.name).toBe(
        'Persisted office',
      );
      expect(tx.dossier.findFirst).toHaveBeenLastCalledWith(
  ...
  it('rejects an office outside the organization or inactive before creating payment', async () => {
    ...
    await expect(
      service.updateStatus(
        'dossier',
        { status: 'depositReceived', deposit },
        'actor',
        'org',
      ),
    ).rejects.toThrow('Bureau');
    expect(tx.payment.create).not.toHaveBeenCalled();
  });
  it.each([DossierType.VEHICLE_SALE_CIF, DossierType.VEHICLE_SALE_DDP])(
    'uses actual shipment relations in %s detail and transitions',
    async (type) => {
      ...
      expect(
        (await service.getAllowedTransitions('dossier', 'org'))
          .allowedTransitions,
      ).toContain('loading');
      ...
      expect(
        (await service.getAllowedTransitions('dossier', 'org'))
          .allowedTransitions,
      ).toContain('shipmentBooking');
      expect(tx.shipment.findFirst).toHaveBeenCalledWith({
        where: {
          organizationId: 'org',
          OR: [
            {
              vehicles: {
                some: {
                  vehicle: {
                    dossierVehicles: { some: { dossierId: 'dossier' } },
                  },
                },
              },
            },
