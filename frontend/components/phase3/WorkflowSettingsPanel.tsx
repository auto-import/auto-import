...
import VehicleLookupCreateDialog from "@/components/commerce/VehicleLookupCreateDialog";
...
export default function WorkflowSettingsPanel() {
  ...
  const [lookupDialog, setLookupDialog] = useState<
    ApiVehicleLookup["kind"] | null
  >(null);
  const [editingLookup, setEditingLookup] = useState<ApiVehicleLookup | null>(
    null,
  );
  ...
  async function saveInsurance() {
    await commerceApi.configuration.updateInsurance(
      insurance ? Number(insurance) : undefined,
    );
  ...
  async function addDuty(event: FormEvent) {
    ...
    await commerceApi.configuration.upsertDuty({
      category: duty.category,
      ratePercent: duty.ratePercent ? Number(duty.ratePercent) : undefined,
    });
  ...
  async function addDelivery(event: FormEvent) {
    event.preventDefault();
    await commerceApi.configuration.upsertDelivery({
      destination: delivery.destination,
      amount: delivery.amount ? Number(delivery.amount) : undefined,
  ...
  function editLookup(item: ApiVehicleLookup) {
    setEditingLookup(item);
    setEditValue(item.value);
    setLookupError("");
  }
  async function saveLookupEdit(event: FormEvent) {
    event.preventDefault();
    if (!editingLookup) return;
    const value = editValue.trim().replace(/\s+/g, " ");
    if (!value) return setLookupError("Le nom est obligatoire.");
    setLookupSaving(true);
    setLookupError("");
    try {
      await commerceApi.configuration.updateLookup(editingLookup.id, { value });
  ...
  async function toggleLookup(item: ApiVehicleLookup) {
    await commerceApi.configuration.updateLookup(item.id, {
      active: !item.active,
    });
    await load();
  }
    ...
    <div className="mx-auto mt-6 max-w-5xl space-y-6">
      ...
      <section className="card">
        ...
        <p className="mt-1 text-sm text-muted">
          Aucune valeur par défaut n’est appliquée. Les calculs restent bloqués
          tant que les taux requis sont absents.
        </p>
        <div className="mt-5 grid gap-4 md:grid-cols-3">
          ...
          <button
            ...
            className="self-end rounded-lg bg-neutral-900 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50"
          >
            Enregistrer l’assurance
          </button>
        </div>
        <div className="mt-6 grid gap-6 md:grid-cols-2">
          <form onSubmit={addDuty} className="rounded-xl border p-4">
            <h3 className="font-semibold">Droits par carrosserie</h3>
            <input
              required
              disabled={!canManageSettings}
              className={`${inputClass} mt-3`}
              placeholder="Catégorie / carrosserie"
              value={duty.category}
              onChange={(e) =>
                setDuty((v) => ({ ...v, category: e.target.value }))
              }
            />
            <input
              disabled={!canManageSettings}
              ...
              min="0"
              step="0.0001"
              placeholder="Taux (%) — non configuré"
              value={duty.ratePercent}
              onChange={(e) =>
                setDuty((v) => ({ ...v, ratePercent: e.target.value }))
              }
            />
            <button
              disabled={!canManageSettings}
            ...
            <ul className="mt-3 text-sm">
              {pricing?.dutyRates.map((rate) => (
                <li key={rate.id}>
                  {rate.category}: {rate.ratePercent ?? "non configuré"}%
                </li>
              ))}
            </ul>
          </form>
          <form onSubmit={addDelivery} className="rounded-xl border p-4">
            <h3 className="font-semibold">Livraison locale</h3>
            <input
              required
              disabled={!canManageSettings}
              className={`${inputClass} mt-3`}
              placeholder="Destination ou DEFAULT"
              value={delivery.destination}
              onChange={(e) =>
                setDelivery((v) => ({ ...v, destination: e.target.value }))
              }
            />
            <div className="mt-3 flex gap-2">
              <input
                ...
                min="0"
                step="0.01"
                placeholder="Montant — non configuré"
                value={delivery.amount}
                onChange={(e) =>
                  setDelivery((v) => ({ ...v, amount: e.target.value }))
                }
              />
              <input
                readOnly
            ...
            <ul className="mt-3 text-sm">
              {pricing?.deliveryRates.map((rate) => (
                <li key={rate.id}>
                  {rate.destination}: {rate.amount ?? "non configuré"}{" "}
                  {rate.currency}
                </li>
              ))}
            </ul>
      ...
      <section className="card">
        ...
        <p className="mt-1 text-sm text-muted">
          Les valeurs désactivées restent liées aux véhicules historiques mais
          disparaissent des nouvelles saisies.
        </p>
        <div className="mt-5 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {(Object.keys(kindLabels) as ApiVehicleLookup["kind"][]).map(
            (kind) => (
              <div key={kind} className="rounded-xl border p-4">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <button
                    type="button"
                    disabled={!canManageLookups}
                    onClick={() => setLookupDialog(kind)}
                ...
                <ul className="mt-3 max-h-56 space-y-2 overflow-auto text-sm">
                  {lookups
                    ...
                    .map((item) => (
                      <li
                        key={item.id}
                        className={`flex items-center justify-between gap-2 ${item.active ? "" : "text-muted line-through"}`}
                      >
                        <button
                          type="button"
                          disabled={!canManageLookups}
                          onClick={() => editLookup(item)}
                          className="text-left"
                        >
                          {item.value}
                          {item.needsReview ? " ⚠" : ""}
                        ...
                        <button
                          type="button"
                          disabled={!canManageLookups}
                          onClick={() => void toggleLookup(item)}
      ...
      {lookupDialog && (
        <VehicleLookupCreateDialog
          kind={lookupDialog}
          lookups={lookups}
          create={commerceApi.configuration.createLookup}
          onClose={() => setLookupDialog(null)}
          onCreated={(created) => {
            setLookups((current) => [...current, created]);
            setMessage(`${created.value} ajouté.`);
            void load();
          }}
        />
      )}
      {editingLookup && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/55 p-4">
          <form
            role="dialog"
            aria-modal="true"
            onSubmit={saveLookupEdit}
            className="card w-full max-w-md space-y-4 p-6"
          >
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold">
                Modifier {editingLookup.value}
              </h2>
              <button
                type="button"
                aria-label="Fermer"
            ...
            <div className="flex justify-end gap-2">
              ...
              >
                {lookupSaving ? "Enregistrement…" : "Enregistrer"}
              </button>
            </div>
          </form>
        </div>
