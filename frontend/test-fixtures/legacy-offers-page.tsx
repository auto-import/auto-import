  ...
  const filtered = useMemo(() => {
    return offres.filter((o) => {
      ...
      const matchesCondition =
        conditionFilter === "tous" || o.type === conditionFilter;
  ...
  const columns: Column<Offre>[] = useMemo(() => {
    const cols: Column<Offre>[] = [
      ...
      },
      {
        key: "annee",
        header: "Année",
        render: (o) => <span className="text-sm">{o.annee}</span>,
      },
      {
        key: "type",
        header: "État",
        render: (o) => (
          <span
            className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${o.type === "neuf" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}
          >
      ...
      },
      {
        ...
        render: (o) => (
          <span className="text-sm font-medium">
            {o.prix_cif.toLocaleString("fr-FR")} {o.devise}
          </span>
        ),
      },
      {
        ...
        render: (o) => (
          <span className="text-sm font-medium">
            {o.prix_ddp.toLocaleString("fr-FR")} {o.devise}
          </span>
        ),
      },
    ];
    ...
    if (canViewPrixAchat) {
      cols.push({
        ...
        render: (o) => (
          <span className="text-sm font-medium">
            {o.prix_achat_interne
              ? `${o.prix_achat_interne.toLocaleString("fr-FR")} ${o.devise}`
              : "—"}
          </span>
        ),
      });
    ...
    cols.push(
      ...
      },
      {
        ...
        render: (o) => (
          <StatusBadge
            label={OFFRE_STATUT_LABELS[o.statut]}
            variant={OFFRE_STATUT_VARIANTS[o.statut]}
          />
        ),
      },
      {
        ...
        render: (o) => (
          <div className="flex items-center gap-1">
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                router.push(`/dossiers/new?offre_id=${o.id}`);
              }}
              className="p-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
              title="Créer dossier"
            >
