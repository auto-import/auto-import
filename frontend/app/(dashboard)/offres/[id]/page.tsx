...
import { getOffreById, getRoleById } from '@/lib/mockData';
import { useAuth } from '@/components/AuthProvider';
import { OFFRE_STATUT_LABELS, OFFRE_STATUT_VARIANTS, formatDate } from '@/lib/constants';
import { ArrowLeft, MapPin, Calendar, Clock, Package, Edit, FolderOpen, ExternalLink, Car } from 'lucide-react';
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <h2 className="text-xl font-semibold text-foreground mb-2">Offre introuvable</h2>
            <p className="text-muted-foreground mb-6">L&apos;offre demandée n&apos;existe pas ou a été supprimée.</p>
            <button
      ...
      <div className="flex-1 overflow-auto p-6 space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
          <div className="space-y-2">
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="text-2xl font-bold text-foreground">{offre.reference}</h1>
              <StatusBadge
            ...
            <p className="text-lg text-foreground">
              {offre.marque} {offre.modele}{offre.version ? ` ${offre.version}` : ''}
            </p>
            <div className="flex items-center gap-3 text-sm text-muted-foreground flex-wrap">
              ...
              <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${offre.type === 'neuf' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                {offre.type === 'neuf' ? 'Neuf' : 'Occasion'}
              </span>
              {offre.fournisseur_nom && (
                <span className="flex items-center gap-1">
                  <MapPin className="w-3.5 h-3.5" />
                  {offre.fournisseur_nom}{offre.ville_fournisseur ? `, ${offre.ville_fournisseur}` : ''}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button
              onClick={() => router.push(`/dossiers/creer?offre_id=${offre.id}`)}
              className="btn-primary inline-flex items-center gap-2"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 card p-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              ...
              <InfoItem label="État" value={offre.type === 'neuf' ? 'Neuf' : 'Occasion'} />
              <InfoItem label="Kilométrage" value={offre.kilometrage ? `${offre.kilometrage.toLocaleString('fr-FR')} km` : '—'} />
          ...
          <div className="card p-6">
            ...
            <div className="space-y-4">
              <div className="rounded-card border border-border p-3">
                <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">Fournisseur</p>
                <a
                  href={offre.fournisseur_id ? `/fournisseurs/${offre.fournisseur_id}` : '#'}
              ...
              <div className="space-y-3">
                <div className="rounded-card border border-border p-3">
                  <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">Prix CIF</p>
                ...
                <div className="rounded-card border border-border p-3">
                  <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">Prix DDP</p>
                  <p className="text-sm font-semibold text-foreground">
                ...
                {canViewPrixAchat && offre.prix_achat_interne != null && (
                  <div className="rounded-card border border-border p-3">
                    <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">Prix d&apos;achat</p>
                    <p className="text-sm font-semibold text-foreground">
                      {offre.prix_achat_interne.toLocaleString('fr-FR')} {offre.devise}
                    </p>
                  </div>
                )}
                ...
                <div className="rounded-card border border-border p-3">
                  <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">Validité</p>
                  <p className="text-sm text-foreground">
                    {offre.date_validite ? formatDate(offre.date_validite) : '—'}
                  </p>
                </div>
        ...
        <div className="card p-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <InfoItem label="Quantité disponible" value={String(offre.quantite_disponible)} />
            <InfoItem label="Disponibilité" value={offre.disponibilite} />
            <InfoItem label="Statut" value={OFFRE_STATUT_LABELS[offre.statut]} />
            <InfoItem label="Date de création" value={formatDate(offre.date_creation)} />
          </div>
        </div>
        ...
        {offre.notes_internes && (
          <div className="card p-6">
            ...
            <p className="text-sm text-foreground whitespace-pre-wrap">{offre.notes_internes}</p>
          </div>
        )}
