...
function LegacyDocumentsHubPage({ onGed }: { onGed: () => void }) {
  ...
  const [clientOptions, setClientOptions] = useState<ApiClient[]>([]);
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      void crmApi
        .listClients({ search: clientSearch || undefined, limit: 20 })
        .then((result) => setClientOptions(result.items))
  ...
  const handleUploadSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedFile || (!uploadDossierId.trim() && !uploadClientId.trim()))
      return;
    ...
    try {
      await uploadDocument(selectedFile, {
        dossierId: uploadDossierId.trim() || undefined,
        clientId: uploadClientId.trim() || undefined,
        kind: uploadKind,
  ...
  const getKindBadge = (kind: string) => {
    switch (kind) {
      ...
      case "PROOF":
        return (
          <StatusBadge variant="green" label="Preuve Paiement" size="sm" />
        );
      case "CUSTOMS_DOCUMENT":
        return <StatusBadge variant="yellow" label="Douane" size="sm" />;
      case "VEHICLE_PHOTO":
        return (
          <StatusBadge variant="purple" label="Photo Véhicule" size="sm" />
        );
      case "BUSINESS_DOCUMENT":
        return (
          <StatusBadge variant="blue" label="Document Commercial" size="sm" />
        );
      default:
    ...
    },
    {
      ...
      render: (row) =>
        row.client ? (
          ...
          <span className="text-muted">Sans client</span>
        ),
    },
    {
      key: "kind",
    ...
    },
    {
      ...
      render: (row) => (
        <button
          type="button"
          onClick={() =>
            void downloadDocument(row.id).catch((error: unknown) =>
              setErrorMsg(
          ...
          <div className="card p-5 border-l-4 border-l-status-yellow-text flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-status-yellow-text mt-1">
                {
                  documents.filter(
                    (d) => d.kind === "PAYMENT_RECEIPT" || d.kind === "PROOF",
                  ).length
                }
              </p>
          ...
          <div className="flex flex-wrap items-center gap-4 flex-1">
            ...
            >
              ...
              <option value="PAYMENT_RECEIPT">
                Preuves & Reçus de paiement
              </option>
              <option value="CUSTOMS_DOCUMENT">
                Documents Douaniers & B/L
              </option>
            ...
            <input
              ...
              className="px-4 py-2.5 text-sm border border-border rounded-input bg-background"
          ...
          <div className="card max-w-md w-full p-6 space-y-4">
            ...
            <form onSubmit={handleUploadSubmit} className="space-y-4">
              ...
              <div>
                ...
                >
                  <option value="">
                    Dérivé automatiquement avec un dossier
                  </option>
                  {clientOptions.map((client) => (
                    <option key={client.id} value={client.id}>
                      {client.firstName} {client.lastName}
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button
                  ...
                  disabled={
                    uploading ||
                    !selectedFile ||
                    (!uploadDossierId.trim() && !uploadClientId.trim())
                  }
